...
export const requirementSearchDescription: INodeProperties[] = [
	{
		...
		description:
			'Raw JSON filter body sent directly to the eRS Cloud <code>/v1/requirements/search</code> endpoint. ' +
			'It must be valid JSON and follow the filter syntax described in the eRS Cloud API docs ' +
			'(<em>Search Requirements</em> section at <code>https://apidocs.eresourcescheduler.cloud/#introduction</code>). ' +
